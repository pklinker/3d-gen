...
function generate(seed: number, p: ParamValues): GeneratedMesh {
  ...
  tube(P, I, [0, mastBaseY, mast.z], [0, mastTopY, mastTopZ], 0.014, 5, false, true);
  const mastEnd = I.length;

  // Pennant: a thin triangular flag streaming aft from near the masthead. Seed varies its
  // length and droop, and cuts a swallowtail notch about half the time.
  const flagStart = I.length;
  {
    ...
    if (rng() < 0.5) {
      // Swallowtail: two tails with a notch between them.
