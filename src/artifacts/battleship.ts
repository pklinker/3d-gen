...
const params = [
  ...
  { key: "clutterDensity", label: "Deck clutter density", kind: "number", min: 0, max: 2, step: 0.1, default: 1 },
  { key: "bowAntenna", label: "Bow antenna",       kind: "bool",   default: false },
...
/** Build a twin-barrelled centerline gun turret. barrelDir = +1 for bow, -1 for stern. */
function gunTurret(
  P: number[], I: number[],
...
/**
 * Build a single-barrelled gun turret molded into the cabin's flank, sitting on the deck.
 * baseX = the turret's centerline (straddling the cabin side wall so it reads as embedded
 * in the deckhouse rather than a hull-side sponson); side = -1 port, +1 starboard.
 */
function sideTurret(
  P: number[], I: number[],
  baseX: number, baseY: number, tz: number, side: number,
): void {
  const tR     = 0.085;
  const tBodyH = 0.06;
  const tBodyR = 0.07;

  frustum(P, I, [baseX, baseY, tz],              [baseX, baseY + 0.03, tz],              tR,           tR * 0.90,   8, true,  false);
  frustum(P, I, [baseX, baseY + 0.03, tz],       [baseX, baseY + 0.03 + tBodyH, tz],    tR * 0.90,    tBodyR * 0.75, 8, false, false);
  dome(P, I, baseX, tz, baseY + 0.03 + tBodyH, tBodyR * 0.75, 2, 8);

  const barrelY  = baseY + 0.03 + tBodyH * 0.55;
  const barrelX0 = baseX + side * 0.01;
  const barrelX1 = baseX + side * 0.24;
  tube(P, I, [barrelX0, barrelY, tz], [barrelX1, barrelY, tz], 0.021, 6, true, false);
  tube(P, I, [barrelX1 - side * 0.025, barrelY, tz], [barrelX1 + side * 0.012, barrelY, tz], 0.027, 6, false, true);
}

/**
 * Battleship: the Cruiser hull, cabin and mast, up-armed with a pair of single-barrelled
 * gun turrets molded into the cabin's flanks, and a third pusher engine astern whose
 * propeller mounts straight onto the hull with no nacelle, unlike the twin outriggers.
 */
function generate(seed: number, p: ParamValues): GeneratedMesh {
  ...
  const clutterDensity = p.clutterDensity as number;
  const bowAntenna = p.bowAntenna as boolean;
  ...
  const cabinSt   = station(0.50);
  const cabinBase = deckAt(cabinSt.z) - 0.01;
  const cabinHw   = 0.16 + 0.06 * cabinSize, cabinHz = 0.13 + 0.14 * cabinSize;
  const cabinH1   = 0.10 + 0.08 * cabinSize;
  {
    const h2 = 0.07 + 0.04 * cabinSize;
    box(P, I, -cabinHw, cabinHw, cabinBase, cabinBase + cabinH1, cabinSt.z - cabinHz, cabinSt.z + cabinHz);
    const bw = cabinHw * 0.7, bz = cabinHz * 0.75;
    box(P, I, -bw, bw, cabinBase + cabinH1, cabinBase + cabinH1 + h2, cabinSt.z - bz, cabinSt.z + bz);
    box(P, I, -bw * 0.85, bw * 0.85, cabinBase + cabinH1 + h2, cabinBase + cabinH1 + h2 + 0.025, cabinSt.z - bz * 0.85, cabinSt.z + bz * 0.85);
  }
  ...
  // ── Wing turrets: a single-barrelled gun molded into each flank of the cabin,
  // sitting on the deck rather than sponsoned off the hull's outer edge ──
  const sideTurretStart = I.length;
  sideTurret(P, I, -cabinHw, cabinBase, cabinSt.z, -1); // port
  sideTurret(P, I,  cabinHw, cabinBase, cabinSt.z,  1); // starboard
  const sideTurretEnd = I.length;

  // ── Mast + pennant (between turrets). Seed jitters masthead height + fore/aft lean. ──
  ...
  // ── Twin rear pusher engines (outriggers) ────────────────────────────────────
  ...
  // ── Center engine: a third pusher whose propeller mounts directly on the hull —
  // no nacelle housing, unlike the twin outrigger engines above.
  const centerEngineStart = I.length;
  {
    const cy0   = sternRing.cy;
    const discZ = sternRing.z - 0.02;
    for (let i = 0; i < blades; i++) {
      const ang = (i / Math.max(1, blades)) * Math.PI * 2;
      const ca = Math.cos(ang), sa = Math.sin(ang);
      const inner: [number, number, number] = [ca * 0.04, cy0 + sa * 0.04, discZ];
      const outer: [number, number, number] = [ca * 0.19, cy0 + sa * 0.19, discZ];
      frustum(P, I, inner, outer, 0.032, 0.012, 4, true, true, ang);
    }
  }
  const centerEngineEnd = I.length;

  // Seeded deck clutter on the clear bow deck, forward of the forward turret.
  // Density is user-controlled; the seed still decides exactly which props land where.
  const clutterStart = I.length;
  const clutterMax = Math.round(3 * clutterDensity);
  if (clutterMax > 0) deckClutter(P, I, rng, station(0.82).z, station(0.94).z, deckAt, halfWAt, clutterMax);
  const clutterEnd = I.length;

  // Bow antenna: a manually-toggled greeble, placed at a fixed station regardless of seed —
  // unlike deckClutter above, it's either present or not, not randomly scattered.
  const antennaStart = I.length;
  if (bowAntenna) {
    const st = station(0.96);
    const base: [number, number, number] = [0, deckAt(st.z), st.z];
    const tip: [number, number, number] = [0, deckAt(st.z) + 0.16, st.z];
    tube(P, I, base, tip, 0.008, 5, true, true);
  }
  const antennaEnd = I.length;
  ...
  paintRange(geo, turretStart, turretEnd, "#8A8F96", 0.90); // centerline gun turrets: steel
  paintRange(geo, sideTurretStart, sideTurretEnd, "#8A8F96", 0.90); // wing turrets: steel
  ...
  paintRange(geo, metalStart,  metalEnd,  "#8A8F96", 0.90); // outrigger nacelles + propellers: steel
...
export const battleshipDef: ArtifactDef = {
  type: "battleship",
  label: "Battleship",
  category: "ships",
  output: "mesh",
  ...
  fileStem: "battleship",
  promptSeed:
    "low-poly Barsoomian battleship airship, wide timber boat hull with a raked bow prow spar, two-tier superstructure cabin, a forward and a rear twin-barrelled gun turret plus single-barrelled wing turrets sponsoned off each broadside, twin rear pusher engines on stern outriggers and a third central pusher engine astern, retro-futuristic, matte, stylized game asset.",
};
